...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        mvprintw(5, 0, "Targets: %d", processes[4]);
        mvprintw(6, 0, "Obstacles: %d", processes[5]);
        ...
        sprintf(slog, "[watchdog] Server: %d , UI: %d, Keyboard: %d, Drone: %d, Targets: %d, Obstacles: %d",
                      processes[0], processes[1], processes[2], processes[3], processes[4], processes[5]);
        fprintf(file, "%s\n", slog);
        fclose(file);
        sem_post(LOGsem);
